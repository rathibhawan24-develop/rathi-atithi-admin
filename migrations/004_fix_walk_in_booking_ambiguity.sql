-- =============================================================================
-- 004 — Hotfix: walk-in booking column ambiguity
-- =============================================================================
-- Migration 002 rebuilt create_walk_in_booking from the repo file (phase3a-walk-in.sql),
-- which had drifted from production. The production version had two safeguards
-- against an ambiguous column reference on RETURNING that the repo version
-- dropped:
--   1. #variable_conflict use_column directive
--   2. Qualified columns: returning bookings.id, bookings.booking_code
--
-- Without these, the function's RETURNS TABLE OUT parameters (booking_id,
-- booking_code) collide with the table column names in RETURNING, producing
-- the error "column reference 'booking_code' is ambiguous" on every walk-in
-- booking attempt.
--
-- This restores both safeguards. The override-aware rate logic and the
-- optional-email logic from migration 002 are preserved verbatim.
-- =============================================================================
...
begin
  ...
  -- QUALIFIED returning columns to avoid collision with RETURNS TABLE OUT params.
  ...
  returning bookings.id, bookings.booking_code
...
comment on function public.create_walk_in_booking is
  'Walk-in booking creation. Optional email. Override-aware nightly rates. '
  'Uses #variable_conflict use_column and qualified RETURNING to avoid '
  'collision between OUT params (booking_id, booking_code) and bookings columns.';
